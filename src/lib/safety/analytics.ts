...
export interface ClassOverview {
    ...
    sectionBreakdown: { section: string; avgScore: number; studentCount: number }[];
    groupBreakdown: { group: string; avgScore: number; studentCount: number }[];
}
...
export async function computeEngagementScore(username: string): Promise<EngagementScore | null> {
    ...
    const { data: user } = await supabase
        .from('allowed_users')
        .select('username, full_name, original_section, original_group, last_login_at, created_at')
    ...
    let riskScore = (reportCount || 0) * 20 + failedLogins * 5;
    if (loginFrequency < 0.1) riskScore += 15; // very inactive
...
export async function getAllStudentEngagement(filters?: {
    ...
    // Batch fetch: all activity logs for last 30 days
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    ...
    for (const student of students) {
        ...
        let score = 0;
        score += Math.min(loginFrequency * 30, 25);
        score += Math.min(quizzes * 5, 20);
        score += Math.min(lessons * 2, 15);
        score += Math.min(avgSessionMinutes * 0.5, 15);
        score += Math.min(uniquePages.size * 1.5, 10);
...
export async function getClassOverview(): Promise<ClassOverview> {
    ...
    for (const s of allStudents) {
        ...
        const existing = sectionMap.get(sec) || { total: 0, count: 0 };
    ...
    for (const s of allStudents) {
        ...
        const existing = groupMap.get(grp) || { total: 0, count: 0 };
        existing.total += s.score;
        existing.count++;
