...
export function FloatingDaVinci() {
    ...
    return (
        <>
            {/* Chat Window - AnimatePresence for exit animation (CSS can't animate unmounting) */}
            <AnimatePresence>
                {isOpen && (
                    <m.div
                        ...
                        className="fixed bottom-20 sm:bottom-24 right-3 sm:right-6 w-[calc(100%-1.5rem)] sm:w-80 md:w-96 h-[60vh] sm:h-[450px] max-h-[500px] bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col origin-bottom-right"
                    >
                    {/* Header */}
            ...
            {/* Floating Button - CSS only for hover/active (no Framer Motion needed) */}
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="fixed bottom-4 sm:bottom-6 right-3 sm:right-6 w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-primary to-secondary rounded-full shadow-lg flex items-center justify-center text-white z-50 hover:shadow-xl transition-transform hover:scale-110 active:scale-95 border border-white/10 will-change-transform"
            >
